function newVals=dirichletboundaryPCG(L,index,vals)
...
num = length(antiIndex);
try
  P = ichol(L(antiIndex,antiIndex), struct('michol','on'));
catch %#ok<CTCH>
  P = speye(num,num);
...
[x,~,~,~,~] = pcg(L(antiIndex,antiIndex),b,1e-6,300,P,P'); %#ok<ASGLU,NASGU>
